...
function initialiseFloatingImages()
{
    ...
    for (var i = 0; i < imageCount; i++) {
        ...
        // set step variables
        thetaStep[i] = 0.1 + Math.random()/10;
        // set step variables
        yStep[i] = 5 + Math.random();
        // write layers etc...
        if (ns4up) {
            document.write('<layer name="dot'+i+'" left="15" top="15" visibility="show"><img src="'+imageSrc+'" alt="Floating image"/></layer>');
        } else if (ie4up||ns6up) {
            document.write('<div id="dot'+i+'" style="POSITION:absolute;  Z-INDEX:'+i+'; VISIBILITY:visible; TOP:15px; LEFT:15px;"><img style="WIDTH:23%;" src="'+imageSrc+'" alt="Floating image"/></div>');
        }
    }
